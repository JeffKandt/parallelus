...
def main() -> None:
    ...
    if not marker_path.exists():
        raise SystemExit(
            "retro_audit_local: marker not found; run .agents/bin/retro-marker first"
        )
    ...
    if marker_head and marker_head != head:
        raise SystemExit(
            ...
            f"(marker={marker_head}, current={head}); rerun .agents/bin/retro-marker"
        )
    ...
    if not failures_path.exists():
        raise SystemExit(
            ...
            "run .agents/bin/collect_failures.py after .agents/bin/retro-marker"
        )
