...
5. Senior-review launch now enforces retrospective freshness: the current marker
...
6. Do not hand-write review markdowns; merges will be blocked unless the review
   provenance indicates a subagent run.
7. The helper refuses to send new instructions to a stale review sandbox once
   the feature branch `HEAD` moves; launch a fresh subagent for each follow-up
   commit instead of reusing the previous pane.
8. Remind the reviewer that their session is read-only apart from the review
   file—no plan/progress edits, `make bootstrap`, or other helpers that change
   the workspace are permitted during the review run.
9. Start monitoring with `make monitor_subagents ARGS="--id <id>"` (rather than
...
10. If the progress log still contains placeholder text ("pending update", TODOs,
