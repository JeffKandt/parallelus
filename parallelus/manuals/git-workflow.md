...
1. If `origin/HEAD` (or `${BASE_REMOTE}/HEAD`) exists, use it to derive the base
   branch.
2. Otherwise fall back to `DEFAULT_BASE` (from `parallelus/engine/agentrc`), then
...
If the repository includes `parallelus/engine/custom/README.md`, read it now and incorporate any project-specific expectations (extra checks, restricted paths, custom adapters) into your plan. Treat those instructions as an extension of this manual.
...
Use `make bootstrap slug=<slug>` (wraps `parallelus/engine/bin/agents-ensure-feature`).
The helper:
...
`make queue_init` creates `parallelus/engine/queue/next-branch.md`, `make queue_show`
...
- The helper refuses to run when any branch notebooks are still present (for
  ...
  `parallelus/engine/bin/fold-progress apply` to fold entries verbatim into the canonical
...
### Archive
Use `make archive b=<branch>` (wraps `parallelus/engine/bin/agents-archive-branch`):
...
2. Fold notebook updates into canonical docs (`docs/PLAN.md`, `docs/PROGRESS.md`
   via `parallelus/engine/bin/fold-progress apply`) while still on the feature branch.
...
- When folding progress logs, preserve every entry verbatim—run `make turn_end`
  ...
  `parallelus/engine/bin/fold-progress apply` and only edit notebook text before folding
