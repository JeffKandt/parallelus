...
4. `parallelus/engine/adapters/python/env.sh` – creates `.venv` (if missing) and installs
   `requirements.txt`.
...
All scripts read configuration knobs from `parallelus/engine/agentrc`.

## Make Targets
Including `parallelus/engine/make/python.mk` adds:
- `make lint`
- `make format`
- `make test`
...
parallelus/engine/adapters/python/env.sh
