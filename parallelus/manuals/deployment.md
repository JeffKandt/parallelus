...
Run `parallelus/engine/bin/deploy_agents_process.sh` to transplant the workflow into another repository. The script can scaffold a brand-new repo or overlay the process onto an existing one.

```
# Create a fresh repository with Python defaults
parallelus/engine/bin/deploy_agents_process.sh ~/Code/my-app

# Scaffold a Swift workspace
parallelus/engine/bin/deploy_agents_process.sh --name my-swift-app --lang swift ~/Code/my-swift-app
...
- `--lang LANG` – add one or more language overlays (`python`, `swift`). Repeat the flag to include several overlays; the script updates `parallelus/engine/agentrc` and the Makefile snippet accordingly.
...
1. Copy the following into the destination root:
   - `AGENTS.md`
   - `parallelus/engine/` (entire directory)
   - `parallelus/manuals/` tree
   ...
   - Create an empty `./.parallelus/sessions/` directory
2. Update `parallelus/engine/agentrc` for the new project (`PROJECT_NAME`, `DEFAULT_BASE`, `LANG_ADAPTERS`, etc.).
...
`parallelus/engine/bin/subagent_manager.sh` to prepare the sandbox, prints the relevant paths,
...
- `SCOPE_TEMPLATE` – scope file copied into the sandbox (default
  `parallelus/engine/prompts/process_smoke.txt`).
...
adapting `parallelus/engine/bin/process_self_test.sh` and `parallelus/engine/bin/subagent_manager.sh` to use
...
- Document how to install the local hooks (copy `parallelus/engine/hooks/pre-merge-commit` or run `make merge` once).
- Encourage running `make ci` before every merge to catch regressions and exercise the smoke suite.

## 6. Optional Enhancements

- Add repository-specific adapters under `parallelus/engine/adapters/` (Go, Rust, etc.).
