...
> header in `parallelus/engine/prompts/agent_roles/senior_architect.md` (defaults in
> the YAML front matter at the top of
> `parallelus/engine/prompts/agent_roles/senior_architect.md`) before launch. The subagent operates
...
- Optional helpers (`parallelus/engine/bin/process_self_test.sh`,
  `scripts/run_with_watchdog.py`) are available if needed for logging/monitoring.

## 3. Command Interface

All orchestration commands are exposed through `parallelus/engine/bin/subagent_manager.sh`
(to be implemented). Interface summary:

> **Socket guard:** Helper scripts route all tmux calls through
> `parallelus/engine/bin/tmux-safe`, which automatically applies `PARALLELUS_TMUX_SOCKET`
...
- `launch --type {throwaway|worktree} --slug <branch-slug> --scope <scope-file>
  ...
  - Drops the scope file, registers the subagent in
    `parallelus/manuals/subagent-registry.json`, and launches Codex via the selected
...
- After launch, the CLI prints a reminder to run
    `parallelus/engine/bin/agents-monitor-loop.sh --id <entry>`; treat this as mandatory
  ...
  - Launches now check whether a tmux session is available; if none is found,
    ...
    `parallelus/engine/bin/resume_in_tmux.sh` so you can resume the session inside the
...
2. Drop an untracked scope file (e.g. `SUBAGENT_SCOPE.md`) describing tasks,
   ...
   `parallelus/manuals/templates/subagent_scope_template.md` as a starting point.
3. Record an entry in `parallelus/manuals/subagent-registry.json` capturing sandbox path,
...
`parallelus/engine/bin/subagent_tail.sh --id <registry-id>` prefers `subagent.last_message.txt`,
...
nudge is necessary, use `parallelus/engine/bin/subagent_send_keys.sh --id <registry-id> --text "Proceed"`
...
1. Run `./parallelus/engine/bin/subagent_manager.sh status --id <registry-id>` to confirm which
...
4. Review the transcript to decide the next step:
   - If the buffer is waiting for input, send any follow-up instructions with `parallelus/engine/bin/subagent_send_keys.sh --id <registry-id> --text "…"`.
...
8. Do not rely on typing `exit` inside the sandbox to close the session—the tmux pane
   ...
   confirm no further alerts, then execute `./parallelus/engine/bin/subagent_manager.sh cleanup --id <registry-id>`
...
9. Generate a human-readable transcript for archives by running
   `./parallelus/engine/bin/subagent_session_to_transcript.py docs/guardrails/runs/<id>/session.jsonl`
...
- [ ] Restart or resume any monitoring loops paused for the subagent (e.g. `parallelus/engine/bin/agents-monitor-loop.sh --id <registry-id>` or your local equivalent) so the main session regains coverage.
- [ ] Record the decision in the branch progress notebook using `parallelus/manuals/templates/subagent_acceptance_snippet.md`, capturing follow-ups or rework requests.
...
- A helper script (`parallelus/engine/bin/subagent_alive.sh`) is available for Codex sessions:
  run `/shell parallelus/engine/bin/subagent_alive.sh` to confirm whether the sandbox
...
- Use `parallelus/engine/bin/agents-monitor-loop.sh` (invoked directly or via
...
- `make start_session` can pause with `parallelus/engine/bin/agents-session-start: line 43: <suffix>: unbound variable` when existing session directories include timestamp suffixes (e.g. `041-20251004155824-a73977`). Strip the suffix before performing `10#$dir` arithmetic in the helper so smoke prompts and harness sessions launch without manual `SESSION_ID` overrides.
...
The default scope template lives at `parallelus/manuals/templates/subagent_scope_template.md`. Copy or customise it when preparing instructions for a subagent. Ensure the final plan/progress notebooks mirror the tasks defined in the scope.
