...
- **Focus:** Changes since the last approved commit (`db67706404e69a312a671f65fc8d62b2e162925d`) are limited to:
  ...
  2. Merge-time enforcement in `parallelus/engine/bin/agents-merge` and `parallelus/engine/hooks/pre-merge-commit` that blocks lingering review notebooks.
...
- [ ] Review the merge-time enforcement updates in `parallelus/engine/bin/agents-merge` and `parallelus/engine/hooks/pre-merge-commit`.
