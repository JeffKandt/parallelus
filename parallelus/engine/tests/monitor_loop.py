#!/usr/bin/env python3
"""Smoke tests for `parallelus/engine/bin/agents-monitor-loop.sh` guardrails.
...
def _write_stub(repo_root: Path, scenario: str) -> None:
    ...
    stub_path = repo_root / "parallelus/engine" / "bin" / "subagent_manager.sh"
...
def _setup_repo(scenario: str) -> Path:
    ...
    script_dest = tmp_dir / "parallelus/engine" / "bin"
...
def _run_monitor(
    ...
    cmd = [str(Path(repo_root, "parallelus/engine", "bin", "agents-monitor-loop.sh")), "--interval", "0", *extra_args]
