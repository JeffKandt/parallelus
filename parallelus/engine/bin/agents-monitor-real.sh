...
SUBAGENT_MANAGER="$ROOT/parallelus/engine/bin/subagent_manager.sh"
...
ENTRY_SUMMARIES=()
SCENARIO_REPORTS=()
SESSION_TRANSCRIPT_TOOL="$ROOT/parallelus/engine/bin/subagent_session_to_transcript.py"
...
evaluate_scenario() {
  ...
  if [[ $actual_success -eq 0 ]]; then
    if [[ -x "$ROOT/parallelus/engine/bin/subagent_tail.sh" ]]; then
      echo "---- transcript tail for $scenario ($entry_id) ----" >&2
      "$ROOT/parallelus/engine/bin/subagent_tail.sh" --id "$entry_id" --lines 40 >&2 || true
