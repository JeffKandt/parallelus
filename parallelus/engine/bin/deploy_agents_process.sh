...
usage() {
    cat <<'USAGE'
Usage: parallelus/engine/bin/deploy_agents_process.sh [OPTIONS] TARGET_DIRECTORY
...
ensure_source_repo() {
    if [[ ! -f "$SOURCE_REPO/AGENTS.md" || ! -d "$SOURCE_REPO/parallelus/engine" ]]; then
...
detect_legacy_parallelus_signals() {
    ...
    # Temporary compatibility: detect pre-reorg installs that still use .agents/.
    local strong_path
    local strong_paths=(
...
backup_existing_git_hooks() {
    ...
    for hook_path in "$git_hooks"/*; do
        ...
        source_hook="$SOURCE_REPO/parallelus/engine/hooks/$hook_name"
...
copy_base_assets() {
    ...
    mkdir -p "$TARGET_DIR/.parallelus/sessions"
...
run_verification() {
    ...
    before_sessions=$(ls .parallelus/sessions 2>/dev/null || true)
    ...
    after_sessions=$(ls .parallelus/sessions 2>/dev/null || true)
    for session in $after_sessions; do
        case " $before_sessions " in
            *" $session "*) ;;
            *) rm -rf ".parallelus/sessions/$session" ;;
        esac
    done
    popd >/dev/null
}
